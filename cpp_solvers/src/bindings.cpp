...
template <typename T> inline std::string extractString(const T &object) {
  return bp::extract<std::string>(bp::str(object));
}

std::shared_ptr<aoc::Result> createResultFromDict(bp::dict values) {
  auto result = std::make_shared<aoc::Result>();
  result->success =
      values.has_key("success") ? bp::extract<bool>(values["success"]) : false;
  if (values.has_key("part_one_solution")) {
    result->part_one_solution = extractString(values["part_one_solution"]);
  }
  if (values.has_key("part_two_solution")) {
    result->part_two_solution = extractString(values["part_two_solution"]);
  }
  if (values.has_key("output")) {
    for (auto i = 0; i < bp::len(values["output"]); ++i) {
      result->output.emplace_back(extractString(values["output"][i]));
    }
  }
  // TODO display
  return result;
...
BOOST_PYTHON_MODULE(aoc) {
  ...
  bp::class_<aoc::Result>("Result")
      ...
      .def("__init__",
           bp::make_constructor(&aoc_bindings::createResultFromDict))
