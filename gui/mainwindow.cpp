...
QString MainWindow::createDefault() {
  ...
  const auto dirpath = rootpath + QString("/solvers/%1").arg(year);
  ...
  if (dir.exists(header)) {
    return "Error: " + dir.absolutePath() + "/" + header + " already exists";
  }

  QString implem =
      QString("puzzle_%1_%2.cpp").arg(year).arg(day, 2, 10, QChar('0'));
  if (dir.exists(implem)) {
    return "Error: " + dir.absolutePath() + "/" + implem + " already exists";
  }
  ...
  if (implem_file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    out << QString("#include <solvers/%1/puzzle_%1_%2.h>\n"
  ...
  if (event_file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    for (const auto &file : dir.entryList())
      if (file.startsWith("puzzle_") and file.endsWith(".h"))
        out << QString("#include <solvers/%1/%2>\n").arg(year).arg(file);
  } else {
  ...
  sources.insert(QString("solvers/%1/puzzle_%1_%2.cpp")
                     .arg(year)
                     .arg(day, 2, 10, QChar('0'))
                     .toStdString());

  headers.insert(QString("solvers/%1/puzzle_%1_%2.h")
                     .arg(year)
                     .arg(day, 2, 10, QChar('0'))
                     .toStdString());
  ...
  for (auto y = ui->m_spin_box_year->minimum();
    ...
    auto filename = QString("solvers/%1/event_%1.h").arg(y);
